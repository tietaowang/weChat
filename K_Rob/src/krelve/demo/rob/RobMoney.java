...
public class RobMoney extends AccessibilityService {
    ...
    /** 红包消息的关键字*/
    static final String HONGBAO_QQ_KEY = "[QQ红包]";
            ...
            if(!texts.isEmpty()) {
                for(CharSequence t : texts) {
                    ...
                    if(text.contains(HONGBAO_TEXT_KEY)) {
                        openNotify(event);
                        break;
                    } else if(text.contains(HONGBAO_QQ_KEY)) {
                        openNotify(event);
                        break;
                    }
                }
	...
	private void returnqqmain() {
		final Intent mHomeIntent = new Intent(Intent.ACTION_MAIN);  
	...
	private void checkKey3() {
        ...
        List<AccessibilityNodeInfo> list = nodeInfo.findAccessibilityNodeInfosByText("赶紧点击拆开吧");
        if(list.isEmpty()) {
            list = nodeInfo.findAccessibilityNodeInfosByText(HONGBAO_QQ_KEY);
            for(AccessibilityNodeInfo n : list) {
                Log.i(TAG, "-->QQ红包:" + n);
                n.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                break;
